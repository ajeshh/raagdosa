# RaagDosa — Command Reference (v10.0)
...
Proposals are generated by `raagdosa-scanner scan`. The `tags` command family lets you review, apply, and undo tag changes from those proposals — separate from the folder move pipeline.

### Review proposals
```bash
raagdosa tags status                            # summary by risk tier and status
raagdosa tags review                            # review all pending proposals
raagdosa tags review --risk safe                # only safe-tier proposals
raagdosa tags review --fix-type whitespace      # only whitespace fixes
raagdosa tags review --folder /path/to/album    # only this folder
raagdosa tags review --auto                     # auto-accept safe proposals above threshold
raagdosa tags review --include-protected        # include title (normally excluded)
```

### Apply accepted proposals
```bash
raagdosa tags apply                             # apply accepted proposals (with confirmation)
raagdosa tags apply --dry-run                   # preview without writing
raagdosa tags apply --max-batch 20              # limit batch size
```

### Undo tag changes
```bash
raagdosa tags undo --last                       # revert most recent apply session
raagdosa tags undo --session <id>               # revert a specific session
raagdosa tags undo                              # list recent apply sessions
```

### Config (`config.yaml` → `tag_fix:`)
- `protected_fields: [title, artist]` — fields never modified by bulk apply
- `max_batch_size: 50` — max proposals per apply batch
- `auto_approve_threshold: 1.0` — confidence required for `--auto` (disabled by default; lower to enable)
- `enabled_fixes:` — list of fix types to process (comment out any to skip)
